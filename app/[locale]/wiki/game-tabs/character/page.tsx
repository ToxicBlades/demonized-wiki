      ...
      <Card className="w-full">
        <CardHeader className="p-4">
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="font-bold text-xl sm:text-2xl">{t.title}</CardTitle>
              <CardDescription className="mt-1 text-sm">{t.description}</CardDescription>
            </div>
          </div>
        </CardHeader>
        <CardContent className="p-3 sm:p-6">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent className="w-[calc(100vw-2rem)] max-w-[350px]">
                {tabs.map((tab) => (
                  <DropdownMenuItem
                    key={tab.id}
                    className={cn("flex cursor-pointer items-center gap-2 py-2", activeTab === tab.id && "bg-accent")}
          ...
          <div className="hidden md:block">
            <Tabs value={activeTab} onValueChange={updateTabAndHash} className="w-full">
                ...
                {tabs.map((tab) => (
                  ...
                  >
                    ...
                    <span className="font-medium text-xs">
                      {tab.id === "mystical-power" ? t.mystical_short : tab.label}
                    </span>
                  </TabsTrigger>
                ))}
              </TabsList>
